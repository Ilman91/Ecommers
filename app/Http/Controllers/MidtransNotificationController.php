...
class MidtransNotificationController extends Controller
{
    ...
    public function handle(Request $request)
    {
        ...
        // Kita gunakan trim() untuk jaga-jaga ada whitespace
        // Ambil bagian sebelum tanda hubung (-) terakhir
        // memotong timestamp agar tidak terkirim ke midtrans
        $cleanOrderId = explode('-', $orderId);
        if (count($cleanOrderId) > 2) {
            array_pop($cleanOrderId); // Hapus elemen terakhir (timestamp)
            $cleanOrderId = implode('-', $cleanOrderId);
        } else {
        $cleanOrderId = $orderId;
        }

        $order = Order::where('order_number', $cleanOrderId)->first();

        // 6. IDEMPOTENCY CHECK
        if (in_array($order->status, ['processing', 'shipped', 'delivered', 'cancelled'])) {
            Log::info("Midtrans Notification: Order already processed", ['order_id' => $cleanOrderId]);
        ...
        switch ($transactionStatus) {
            ...
            default:
                Log::info("Midtrans Notification: Unknown status", ['order_id' => $cleanOrderId, 'status' => $transactionStatus]);
        }
