...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white py-3">
        ...
        <ul class="nav nav-pills card-header-pills">
            ...
            <li class="nav-item">
                <a class="nav-link {{ (request('status') == 'completed' || request('status') == 'delivered') ? 'active' : '' }}" href="{{ route('admin.orders.index', ['status' => 'delivered']) }}">Selesai</a>
            </li>
            <li class="nav-item">
                ...
                <tbody>
                    @forelse($orders as $order)
                        <tr>
                            ...
                            <td class="fw-bold">
                                <div class="text-primary">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</div>
                                <small class="text-muted" style="font-size: 0.75rem;">{{ $order->items->count() }} Item</small>
                            </td>
                            <td>
                                @if($order->status == 'pending')
                                    <span class="badge bg-warning text-dark">Pending</span>
                                @elseif($order->status == 'processing')
                                    <span class="badge bg-prymary text-dark">Diproses</span>
